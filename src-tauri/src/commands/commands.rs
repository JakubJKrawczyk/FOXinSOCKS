            ...
            for mut task_clone in auto_run_tasks {
                ...
                if res.is_err() {
                    return Err(String::from(
                        "Błąd uruchamiania tasków podczas inicjalizacji!",
                    ));
                }
            }
            Ok(())
...
pub async fn get_tasks() -> Result<Vec<TaskModel>, String> {
    if INITIALIZED.load(Ordering::SeqCst) {
        ...
        Err(String::from(
            "Rust nie został zainicjalizowany! użyj metody init żeby to zmienić",
        ))
    }
}
...
pub async fn add_task() -> Result<TaskModel, String> {
    if INITIALIZED.load(Ordering::SeqCst) {
        ...
        logger::log(format!(
            "Dodawanie nowego taska id={} title={}",
            task_to_add.id, task_to_add.title
        ));
        let res = CONTROLLER
            .get()
            .unwrap()
        ...
        match res {
            Ok(_) => {
                logger::log("Dodano task");
                Ok(task_to_add)
            }
            Err(e) => {
                logger::error(format!("Błąd dodawania taska: {}", e));
                Err(format!("Nie udało się dodać nowego taska! {}", e))
            }
        }
    } else {
...
pub async fn del_task(task_id: String) -> Result<(), String> {
    if INITIALIZED.load(Ordering::SeqCst) {
        ...
        match res {
            Ok(_) => {
                logger::log("Usunięto task");
                Ok(())
            }
            Err(e) => {
                logger::error(format!("Błąd usuwania taska: {}", e));
                Err(format!("Nie udało się usunąć taska: {}", e))
            }
        }
    } else {
    ...
    if INITIALIZED.load(Ordering::SeqCst) {
        ...
        if let Some(mut task_ref) = task {
            if task_ref.folder_path == "" || task_ref.regex_patterns.len() == 0 {
                ...
                return Err(String::from(
                    "Aby uruchomić zadanie należy uzupełnić ścieżkę do folderu, wwzorce folderów.",
                ));
            }
            let mut c = CONTROLLER.get().unwrap().lock().unwrap();
            match c.create_process(&mut task_ref) {
                Ok(_) => {
                    logger::log("Proces dla taska uruchomiony");
                    Ok(())
                }
                Err(e) => {
                    logger::error(format!("Błąd tworzenia procesu: {}", e));
                    Err(String::from("Błąd tworzenia procesu dla taska"))
                }
            }
        } else {
            logger::warning(format!(
                "Nie znaleziono taska id={} przy uruchamianiu",
                task_id
            ));
            Err(format!("Nie znaleziono zadania dla id {}", task_id))
        }
    ...
    if INITIALIZED.load(Ordering::SeqCst) {
        ...
        if task_exists {
            ...
            match c.stop_process(&task_id) {
                ...
                Err(e) => {
                    logger::error(format!("Błąd zatrzymywania procesu: {}", e));
                    Err(String::from("Błąd zatrzymywania procesu dla taska"))
                }
            }
        } else {
            logger::warning(format!(
                "Nie znaleziono taska id={} przy zatrzymywaniu",
                task_id
            ));
            Err(format!("Nie znaleziono zadania dla id {}", task_id))
        }
        ...
        if let Some(old_task) = backend_task {
            ...
            if let Some(c) = CONTROLLER.get() {
                ...
                if let Err(e) = stop_res {
                    logger::warning(format!(
                        "Nie udało się zatrzymać procesu przed aktualizacją: {}",
                        e
                    ));
                }
            }
            if let Some(c) = CONTROLLER.get() {
                ...
                match update_res {
                    ...
                    Err(e) => {
                        logger::error(format!("Błąd aktualizacji taska: {}", e));
                        Err(String::from("Błąd przy aktualizacji taska"))
                    }
                }
            } else {
            ...
            logger::warning(format!(
                "Nie znaleziono taska id={} do aktualizacji",
                task.id
            ));
